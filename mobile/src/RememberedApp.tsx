...
export default function RememberedApp() {
  ...
  const [currentId, setCurrentId] = useState<string>(
    () => readMemorialIdFromUrl() || 'albert-einstein'
  );
  const [viewMode, setViewMode] = useState<'home' | 'profile'>(() =>
    readMemorialIdFromUrl() ? 'profile' : 'home'
  );
  ...
  useEffect(() => {
    ...
    // No address bar inside the app shell; this is a no-op on a device.
    syncUrlToMemorial();
